class VerifyUserAuthUseCase {
  ...
  async execute(refreshToken) {
    try {
      ...
      const result = await this._cacheService.get(`daundatauser:${datauid.username}`);
      ...
      await this._cacheService.delete(`daundatauser:${datauid.username}`);
      await this._cacheService.set(`daundatauser:${datauid.username}`, JSON.stringify(combinedData));
      return combinedData;
    }
  }
